        ...
        <form class="form-horizontal protoform" id="metadataForm" method="post" action="index.php"
            ...
            {if $nbSample > 0}<fieldset disabled>{/if}
                ...
                <div class="form-group">
                    <label for="metadatatypeName" class="control-label col-md-4"><span class="red">*</span>
                        {t}Nom du modèle :{/t}
                    </label>
                    <div class="col-md-8">
                        <input id="metadatatypeName" type="text" class="form-control" name="metadatatype_name"
                            value="{$data.metadatatype_name}" required autofocus>
                    </div>
                </div>
                <div class="form-group">
                    <label for="metadatatypeComment" class="control-label col-md-4">{t}Description :{/t}</label>
                    <div class="col-md-8">
                        <textarea id="metadatatypeComment" type="text" class="form-control"
                            name="metadatatype_comment">{$data.metadatatype_comment}</textarea>
                    </div>
                </div>
                <div class="form-group">
                    <label for="isArray" class="control-label col-md-4"><span class="red">*</span>
                        {t}Les données sont-elles en tableau (multivaleurs) ?{/t}
                    </label>
                    <div id="isArray" class="col-md-8">
                        <label class="radio-inline">
                            <input type="radio" name="is_array" id="isArray1" value="1" {if $data.is_array == 't'}checked
                                {/if}>
                            {t}oui{/t}
                        </label>
                        <label class="radio-inline">
                            <input type="radio" name="is_array" id="isArray2" value="0" {if $data.is_array != 't'}checked {/if}>
                            {t}non{/t}
                        </label>
                    </div>
                </div>
