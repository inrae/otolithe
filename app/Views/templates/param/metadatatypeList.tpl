...
<div class="row">
	<div class="col-md-6">
		...
		<form method="POST" id="metadataExport" action="metadatatypeExport">
			<div class="row">
				<div class="center">
					{t}Exporter les métadonnées :{/t} <label id="lmetadatacheck" for="checkMetadata">
						{t}Tout décocher{/t}</label> <input type="checkbox" id="checkMetadata" checked>
					<button type="submit" class="btn btn-primary">
						{t}Déclencher l'export{/t}
					</button>
				</div>
			</div>
			...
			<table id="metadataList" class="table table-bordered table-hover datatable" data-order='[[1,"asc"]]'>
				...
				<tbody>
					...
					<tr>
						...
						<td class="center">
							{if $data[lst].is_array == 't'}{t}oui{/t}{else}{t}non{/t}{/if}
						</td>
						{if $rights.param == 1}
						<td class="center">
							<a href="metadatatypeChange?metadatatype_id={$data[lst].metadatatype_id}"
								title="{t}Modifier le modèle de métadonnées{/t}">
								<img src="display/images/edit.png" height="25">
							</a>
						<td class="center">
							<a href="metadatatypeCopy?metadatatype_id={$data[lst].metadatatype_id}"
						...
						{/if}
						<td class="center">
							<input type="checkbox" class="checkMetadata" name="metadatatype_id[]"
								value="{$data[lst].metadatatype_id}" checked>
						</td>
					</tr>
					{/section}
				</tbody>
...
<div class="row col-md-6">
	<fieldset>
		...
		<form class="form-horizontal" id="metadatatypeImport" method="post" action="metadatatypeImport"
